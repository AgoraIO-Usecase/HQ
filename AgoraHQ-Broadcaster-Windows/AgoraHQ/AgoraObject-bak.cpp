...
BOOL CAgoraObject::EnableScreenCapture(HWND hWnd, int nCapFPS, LPCRECT lpCapRect, BOOL bEnable)
{
	...
	if (bEnable) {
		if (lpCapRect == NULL)
			ret = m_lpAgoraEngine->startScreenCapture(hWnd, nCapFPS, NULL, 0);
		else {
			...
			ret = m_lpAgoraEngine->startScreenCapture(hWnd, nCapFPS, &rcCap, 0);
		}
	}
	else
...
BOOL CAgoraObject::SetLogFilter(unsigned int logFilterType, LPCTSTR lpLogPath)
{
	int nRet = 0;
	RtcEngineParameters rep(*m_lpAgoraEngine);
