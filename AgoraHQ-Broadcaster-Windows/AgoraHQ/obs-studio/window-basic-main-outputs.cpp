...
struct SimpleOutput : BasicOutputHandler {
	...
	SimpleOutput(CAgoraHQDlg *main_);
...
SimpleOutput::SimpleOutput(CAgoraHQDlg *main_) : BasicOutputHandler(main_)
{
	const char *encoder = config_get_string(main->Config(), "SimpleOutput",
			"StreamEncoder");
...
void SimpleOutput::UpdateRecordingSettings()
{
 ...
 void SimpleOutput::SetupOutputs()
{
	SimpleOutput::Update();
...
// 					QSystemTrayIcon::Warning);
	...
	if (updateReplayBuffer) {
		...
		obs_data_set_string(settings, "exteAttemptToResetVideonsion", format);
...
struct AdvancedOutput : BasicOutputHandler {
	...
	AdvancedOutput(CAgoraHQDlg *main_);
...
// 	int ret = theApp.GetMainWindow()->GetProfilePath(fullPath, sizeof(fullPath), jsonFile);
// 	if (ret > 0) {
...
AdvancedOutput::AdvancedOutput(CAgoraHQDlg *main_) : BasicOutputHandler(main_)
{
