...
{
	obs_service_t *service = main->GetAgoraService();
	obs_service_t* mainService = main->GetService();
	const char* type = obs_service_get_type(mainService);
	if (strcmp(type, "rtmp_custom") != 0)
		return;

	//only update agora service when rtmp_custon is used
	obs_data_t* settings = obs_service_get_settings(service);
	if (stream1Changed){
		obs_data_set_string(settings, "agora_url", obs_service_get_url(mainService));
		obs_data_set_string(settings, "agora_key", obs_service_get_key(mainService));
	}
	...
	if (outputsChanged){
		...
		int videoBitrate = videoBitrate = config_get_uint(main->Config(), OutputKey.c_str(),
			"VBitrate");
		int  audioBitrate = videoBitrate = config_get_uint(main->Config(), OutputKey.c_str(),
			"ABitrate");

		obs_data_set_int(settings, "agora_video_bitrate", videoBitrate);
		obs_data_set_int(settings, "agora_audio_bitrate", videoBitrate);
	}

	if (videoChanged){

 		int output_cx = config_get_uint(main->Config(), "Video", "OutputCX");
 		int output_cy = config_get_uint(main->Config(), "Video", "OutputCY");
		obs_data_set_int(settings, "agora_out_cx", output_cx);
